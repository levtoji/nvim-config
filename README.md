...
- **Debugging:** [nvim-dap](https://github.com/mfussenegger/nvim-dap) + nvim-dap-ui, `delve` (Go), `netcoredbg` (.NET), `js-debug-adapter` (Node/TypeScript, Adapter-Type `pwa-node`)
- **Run/Debug-Configs (Rider-Paritaet):** `nvim-dap` liest beim Start und bei jedem `DirChanged` (Projektwechsel via neovim-project) eine `.vscode/launch.json` aus dem cwd und haengt deren Eintraege an die eingebauten Fallback-Configs an – `F9` zeigt dann eine Auswahl aller passenden Configs fuer den aktuellen Dateityp (`cs`, `go`, `typescript`/`javascript`). `launch.json` ist projektlokal (nicht Teil dieser Config), Adapter-Types muessen `coreclr`/`go`/`pwa-node` sein, damit sie erkannt werden. Ohne `launch.json` bleiben die Fallbacks aktiv (.NET fragt nach dem DLL-Pfad, Go nutzt dap-go, Node fragt/attacht auf `${file}`).
