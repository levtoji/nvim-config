...
- **Run/Debug-Configs (Rider-Paritaet):** `nvim-dap` liest bei jedem `F9` automatisch (eingebauter `dap.launch.json`-Provider) eine `.vscode/launch.json` aus dem cwd und zeigt deren Eintraege zusammen mit den eingebauten Fallbacks zur Auswahl an. `launch.json` ist projektlokal (nicht Teil dieser Config, meist gitignored), Adapter-Types muessen `coreclr`/`go`/`pwa-node` sein. Ohne `launch.json` bleiben die Fallbacks aktiv (.NET fragt nach dem DLL-Pfad, Go nutzt dap-go, Node fragt/attacht auf `${file}` oder fragt einen npm-Skriptnamen ab).
